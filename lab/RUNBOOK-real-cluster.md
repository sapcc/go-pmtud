...
## Step 2: Deploy RBAC and DaemonSet
  ...
  template:
    ...
    spec:
      ...
      - name: go-pmtud
        ...
        args: 
        - --relay-backend=udp   # explicit: default is l2 (raw Ethernet); use udp for cross-L3 clusters
        securityContext:
          privileged: true
        env:
        - name: NODE_NAME
...
### Inspect Relay Metrics

Check for packet counters (requires metrics scrape or manual inspection):
...
- **Node-2 (peer):** PMTU cache updated via relay (UDP in this example)
...
**Relayed packets not arriving:** Verify RBAC permissions; for the `udp` backend also confirm that UDP port 4390 is reachable between nodes.
