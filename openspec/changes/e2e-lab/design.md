...
replicated to a non-originating peer (for the `udp` relay
...
│                                                                                │
│  go-pmtud DaemonSet on all nodes (udp backend)                                 │
└────────────────────────────────────────────────────────────────────────────┘
```
...
| `cluster.go` | Kind Go API create/delete; isolated temp kubeconfig; `controller-runtime` client (core scheme); worker + control-plane container discovery via `docker ps --filter label=io.x-k8s.kind.role=...`. |
...
| `deploy.go` | `DeployBackend(backend)`: build image, `kind load`, apply RBAC + daemonset (inject `--relay-backend`), `waitRollout`. |
...
`lab/manifests/pmtud-daemonset.yaml`, `lab/manifests/rbac.yaml`. No router image, no `podinfo`, no
...
`config_test.go` — assert the deployed DaemonSet carries the
`--relay-backend=udp` arg.
...
2. **Deploy** — build/push the image; apply RBAC; apply the DaemonSet with
   `--relay-backend=udp`; wait for rollout.
...
4. **Observe replication** — on a **non-originating** node, `ip route get <dst>`
   ...
   `"ICMP frag-needed received, resending packet."` log line appears.
...
2. `go test ./...` (no tag) green — the suite is excluded, so unit CI needs no
   docker/kind.
3. `make -C lab e2e` passes the `udp` backend on Linux **and** macOS
   Docker Desktop.
