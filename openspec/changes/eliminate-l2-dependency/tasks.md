...
- [x] 11.2 Update `go mod tidy` after adding TUN dependency (custom netlink injector via raw ioctl)
...
> **Note**: Tasks 1–15 implemented the initial udp-only cutover (L2 fully removed). The coexistence requirement (existing L2/VLAN installs must stay untouched) supersedes that: the L2 backend is reintroduced as the default, and UDP becomes optional. Tasks 16+ cover this adaptation.

## 16. Relay interface: two backends, no shared injector

- [x] 16.1 Change `Relay.Start` signature from `Start(ctx, inject func([]byte) error) error` to `Start(ctx) error` in `internal/relay/relay.go`
- [x] 16.2 Delete `internal/relay/runnable.go` (daemon-owned injector wrapper)
- [x] 16.3 Move `inject_linux.go`, `inject_other.go`, and `TUNDeviceName` from `internal/relay` into `internal/relay/udp` (Linux build tag preserved)

## 17. Config and CLI: restore L2 flags, default to l2

- [x] 17.1 Extend `config.Backend.Set` to accept `l2` and `udp`; add `BackendL2` const
- [x] 17.2 Restore L2 `Config` fields: `InterfaceNames`/`ReplicationInterface`, `InterfaceMtu`, ARP cache/request timeouts, and MAC cache storage
- [x] 17.3 Re-add CLI flags `--iface_names`, `--iface_mtu`, `--node-timeout-minutes`, `--arp-timeout-seconds` in `internal/cmd/command.go`
- [x] 17.4 Set default `cfg.RelayBackend = conf.BackendL2`; update flag usage string to `l2|udp`
- [x] 17.5 Restore replication-interface resolution in `preRunRootCmd` for L2 mode (from `--iface_names`)

## 18. Resurrect ARP package

- [x] 18.1 Restore `internal/arp/` (from git `e9ebdfe^`), adapting it to resolve MACs from peer IPs on the replication interface
- [x] 18.2 Restore ARP-related metrics if removed (e.g. resolve errors)

## 19. L2 relay backend

- [x] 19.1 Create `internal/relay/l2` implementing `Relay`: `Send` builds Ethernet frames (EtherType 0x0800, payload = raw IP packet) to each peer MAC on the replication interface via a raw packet socket
- [x] 19.2 L2 `Send` resolves peer MACs from `cfg.PeerList` IPs via the ARP package, with a MAC cache honoring the ARP timeouts
- [x] 19.3 L2 `Start(ctx)` sets up the send socket / ARP state, creates no TUN device, and blocks until `ctx.Done()`
- [x] 19.4 Re-add `github.com/mdlayher/arp`, `ethernet`, `packet` to go.mod; `go mod tidy`

## 20. UDP backend + command wiring

- [x] 20.1 Update UDP backend `Start(ctx)` to create/own its `pmtud0` TUN injector internally and tear it down on return
- [x] 20.2 In `internal/cmd/command.go`, add `case conf.BackendL2` to the backend switch; add the backend to the manager directly via `mgr.Add(relayBackend)` (backend is the Runnable)
- [x] 20.3 Log the required NFLOG iptables rule for the active backend at startup (`-i <iface>` for L2, `! -i pmtud0` for UDP)

## 21. Docs

- [x] 21.1 Update README/CONTRIBUTING/runbook: document both backends, `--relay-backend` (default `l2`), per-backend iptables rules, UDP port requirement, and per-backend capability needs
- [x] 21.2 Document the migration path for existing L2 installs (no change) and for opting into UDP

## 22. Tests and validation

- [x] 22.1 Unit test: `--relay-backend` defaults to `l2`; accepts `l2` and `udp`; rejects others
- [x] 22.2 Unit test: L2 backend `Send` writes a frame per peer MAC; `Start` creates no TUN device
- [x] 22.3 Unit test: ARP MAC resolution/caching from peer IPs
- [x] 22.4 Verify existing UDP-backend tests still pass with the new `Start(ctx)` signature
- [x] 22.5 `go build ./...`, `go test ./...`, `go vet ./...` all pass on Linux and macOS
