...
**Files:**
- Modify: `lab/scripts/deploy-pmtud.sh` (accept `RELAY_BACKEND`, reference generated CRD + install RBAC when `crd`)
...
In `deploy-pmtud.sh`, when `RELAY_BACKEND=crd`: `kubectl apply -f $REPO_ROOT/crd/pmtud.cloud.sap_pmtunoderelays.yaml` (the generated CRD) and the namespaced Role/RoleBinding before applying the daemonset; substitute the backend into the daemonset manifest. This avoids manual duplication—the CRD is auto-generated by `make generate` and deployed directly from source.
